...
public class ZKFPDemo extends JFrame{
	...
	public void launchFrame(){
		...
		this.setTitle("Beannsofts ZKFingerSDK Integration via Mpesa");
		...
		btnOpen.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (0 != mhDevice) //mhDevice is a long int || 0 means devise is connected. 
				{ // here we are trying to open the devise or not. 
					//already initialised
					textArea.setText("Please close device first!\n"); 
					return;
				}
				int ret = FingerprintSensorErrorCode.ZKFP_ERR_OK; //error code memory || of type int
				//Initialize
				cbRegTemp = 0;
				bRegister = false;
				bIdentify = false;
				iFid = 1;
				enroll_idx = 0;
                                //Here below we then try to initialize the devise 
				if (FingerprintSensorErrorCode.ZKFP_ERR_OK != FingerprintSensorEx.Init())
				{
					textArea.setText("Init failed!\n");  
					return;
				}
                                // Here we check if any devices connected 
				...
				if (ret < 0)
				{
					...
					return;
                                ...
                                //mhDevice is a long int || 0 means devise is connected.
				if (0 == (mhDevice = FingerprintSensorEx.OpenDevice(0)))
				{
					...
					return;
                                ...
                                //mhDB is a long int | on top it was initialised to (0) 
                                //|| 0 means devise is connected.
                                //DBInit function is used to Initialize FingerPrint Algorithm library.
                                //FingerPrint Algorithm has to be initialised to put it into use.
				if (0 == (mhDB = FingerprintSensorEx.DBInit()))
				{
                                    // ret is of type int but pulled from the fingerprint error value 
			...
			//public static int SetParameters(long devHandle, int code, byte[], paramValue, int size)
                                ...
                                //code-------------Parameter code (See the Appendixes.)
                                //e.g. 2002 implies Anti-fake function (1:enable; 0: disable)
		...
		btnRegImg.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				//String path = "d:\\test\\fingerprint.bmp";
                                // Here we can get the template(fingerprint as a pictuees) from online; 
                                //download it
                                // Save it somewhere and activate it for a user  to  sign in with. 
                                // If user hasnt paid, the system from online will rename the fingerprint on the device , 
                                // names captured from online portal will be used to update the local database 
                                //That's all
                                String path = "C:\\Users\\Mbatha--PC\\Pictures\\test.bmp";
		...
		btnIdentImg.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				//String path = "C:\\Users\\Mbatha--PC\\Pictures\\fingerprint.bmp";
                                String path = "C:\\Users\\Mbatha--PC\\Pictures\\test.bmp";
				byte[] fpTemplate = new byte[2048];
				int[] sizeFPTemp = new int[1];
				sizeFPTemp[0] = 2048;
